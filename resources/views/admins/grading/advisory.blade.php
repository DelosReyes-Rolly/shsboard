...
<main>
            ...
            <div class="card-body p-0" style="padding: 20px 20px 20px 20px;">
                ...
                <div class="panel">
                    <div style="padding-bottom: 400px;">
                        ...
                        <div>
                            <a class="btn btn-primary btn-lg" href="javascript:void(0)" onClick="firstFunc({{ $graderelease->schoolyear_id }})" data-original-title="First" style="float: right; font-size: 18px;"><i class="fas fa-file-alt"></i> Release 1st quarter grades</a>
                            <!-- boostrap release model  -->
                            <div class="modal fade" id="First-modal-delete" aria-hidden="true">
                                <div class="modal-dialog modal-lg">
                                    <div class="modal-content border-start-lg border-start-yellow">
                                        <div class="modal-header">
                                            <h1 class="modal-title" id="FirstModalDelete" style="font-size: 20px;">Release First Grading Grades</h1>
                                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                                <span aria-hidden="true">&times;</span>
                                            </button>
                                        </div>
                                        <form action="javascript:void(0)" id="FirstFormDelete" name="FirstFormDelete" class="form-horizontal" method="POST">
                                            <div class="modal-body">
                                                <input type="hidden" name="id" id="id-first">
                                                <p style="color: red; font-size:20px;">Are you sure you want to release first grading for s.y. <span id="first-release"> </span> ?</p>
                                            </div>
                                            <div class="modal-footer">
                                                <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                                                <font face="Verdana" size="2"><input type="submit" class="btn btn-danger btn-md" value="Release"></font>
                                            </div>
                                        </form>
                                    </div>
                                </div>
    ...
    <script>
        function firstFunc(id) {
            $.ajax({
                type: "POST",
                url: "{{ url('/firstrelease') }}",
                data: {
                    id: id
                },
                dataType: 'json',
                success: function(res) {
                    $('#First-modal-delete').modal('show');
                    $('#id-first').val(res.id);
                    document.getElementById('first-release').innerHTML = res.schoolyear;
                }
            });
        }

        $('#FirstFormDelete').submit(function(e) {
            e.preventDefault();
            var formData = new FormData(this);
            $.ajax({
                type: 'POST',
                url: "{{ url('/firstquarter')}}/",
                data: formData,
                cache: false,
                contentType: false,
                processData: false,
                success: (data) => {
                    $("#First-modal-delete").modal('hide');
                    $("#btn-save").html('Submit');
                    $("#btn-save").attr("disabled", false);
                    Swal.fire({
                        icon: 'success',
                        title: 'Success.',
                        text: 'First Grading grades can be released.',
                    });
                },
                error: function(data) {
                    console.log(data);
